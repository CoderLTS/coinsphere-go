...
`coinsphere-executor` 默认只消费 PostgreSQL 中的 Paper 意图。Testnet 私有验证、首次对账和确定性主市价单默认关闭，只有显式启用并提供与 Backend 相同的安全加密主密钥时才会解密数据库中的 Testnet 凭据；凭据不得写入环境文件，对账不会自动恢复账户，只有账户经用户手工恢复且风控与授权再次通过后才会执行 Testnet 意图，Live 私有接口始终不可用。启动前必须先执行 migration，并为 Executor 配置专用的 PostgreSQL 身份。
