...
- 当前仍保留应用启动时的 GORM AutoMigrate；版本化 migration 已包含机制基线、未进入生产消费的 `worker_tasks` 和 `00003` Outbox schema 契约，不能替代 A1-10 的完整业务 schema 切换。A1-3 不修改或触发 Release 工作流，Outbox dispatcher 运行时也未随该迁移启用。
