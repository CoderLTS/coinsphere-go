...
本文面向当前 V2 个人自托管用户和管理员。当前可用能力是登录、RBAC、系统管理、系统监控、可信本地插件维护，以及超级管理员工作流工作台与批次执行；完整活动/制品、Quant、回测与 Paper 尚未交付。
...
| 角色管理     | 创建角色并分配菜单、按钮权限                             |
| 菜单管理     | 维护后台导航与权限码                                     |
| 工作流工作台 | 创建批处理工作流，编辑 Schema 图、保存修订并手工运行     |

旧行情、新闻、策略、通知和交易页面不属于当前 Web 运行面。旧 `/scheduler/workflow/*` 路径和旧图格式不恢复；新工作台固定使用 `/workflows`。

## 5. 工作流工作台

只有超级管理员可以访问“工作流工作台”和 `/api/v1/workflows`。`blank` 模板使用手工触发，`scheduled` 模板使用 UTC 固定秒数间隔触发；二者都会原子创建一个 `paused` 批处理工作流、初始不可变修订和唯一运行实例。桌面端左侧选择工作流和节点，中间连接 X6 画布，右侧按节点 JSON Schema/UI Schema 编辑配置、上游字段映射、CEL 和密钥；顶部显式保存、启动、暂停、归档或立即运行手工工作流。

保存完整校验通过后才创建新修订。密钥输入只表示替换，留空保持原值；系统只显示是否已配置。切换到历史修订时工作台只读，移动端也只显示状态和修订活动。

启动后，手工工作流可用“立即运行”创建固定当前修订的批次；定时工作流按 `everySeconds` 在 UTC 时间线上去重入队。失败节点默认重试三次并从最后成功检查点继续，取消通过 API 协作停止，暂停会在当前节点完成后停止后续调度。归档后工作流只读且不能重新启动。

具体请求、图格式和冲突语义见[公共契约](contracts/README.md)。工作台不要求管理员编辑原始 JSON；完整节点时间线、日志和历史制品仍由 P1-D 交付。
