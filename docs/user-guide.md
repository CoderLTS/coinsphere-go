...
本文面向当前 V2 个人自托管用户和管理员。当前可用能力是登录、RBAC、系统管理、系统监控、可信本地插件维护，以及超级管理员批处理工作台、活动历史和制品；事件流、Quant、回测与 Paper 尚未交付。
...
桌面工作台底部“运行活动”按游标刷新最近状态。选择带批次编号的活动可查看节点尝试、执行池、耗时、错误类别和制品；下载制品前后分别由服务端和浏览器校验 SHA-256。活动只显示受控摘要，不包含原始载荷、输出或密钥。终态执行历史默认保留 30 天。
