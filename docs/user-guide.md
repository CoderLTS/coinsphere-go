...
默认密码只用于首次种子创建。登录后在工作台使用 `admin.user` 节点创建密码修改任务，经人工确认后为 `coinsphere` 设置至少 12 位的唯一密码。

没有公开注册入口。需要新增用户时，由管理员在工作台使用 `admin.user`、`admin.role` 和 `admin.permissions` 节点处理。

### 4.2 忘记管理员密码

若仍有另一名管理员，通过工作台的 `admin.user` 人工任务重置。完全无法登录时，在能安全连接数据库的环境中使用一次性 CLI；密码只从交互式标准输入读取，不要放进命令行参数：
...
| 工作流工作台 | 创建、配置、校验、激活和运行工作流，处理失败、回测等待、安全审批和业务配置 |
| `/workflows/{id}` | 可分享的桌面编辑与运行深链；移动端为只读观察和轻操作 |
| `/runs/{id}` | 查看节点日志、实际边流转、脱敏输入输出、耗时、重试和结构化错误 |
| 个人中心 | 查看当前用户资料 |

首页与工作台是保留的两个产品入口。旧交易、策略、数据、配置和系统管理菜单不再注册；对应能力由权限可见的节点提供，后端 RBAC 仍是最终安全边界。
...
1. 进入工作台并创建一个手工工作流。
2. 拖入 `trading.account` 节点，操作选择创建。
3. 在结构化表单中填写账户名称，环境选择 `Paper`，市场选择 `Spot` 或 `USD-M`。
4. 设置初始余额、Paper 手续费率、品种白名单和全部风险上限。
5. 校验、保存并运行，在执行覆盖层确认结果。
...
1. 在工作流中添加 `trading.risk` 节点并通过资源选择器选择账户。
2. 在 Decimal 表单中填写完整的新限制并校验。
3. 降低限额可自动执行；任一字段扩大风险时，执行会进入人工待办。
4. 从待办抽屉打开任务，重新认证并确认；随后在执行时间线检查结果。
...
1. 先通过账户节点输出或既有领域只读 API 检查暂停原因、余额、持仓、开放订单和交易意图。
2. 在 `trading.account` 节点选择恢复并运行。
3. 从人工待办重新认证并确认。
4. 在执行详情确认账户变为活动状态。
...
`trading.automation` 和 `trading.authorization` 节点分别处理 Owner 开关与管理员授权。自动执行需要同时满足账户活动、完整风控、无全局急停、状态一致，以及对应的用户与管理员放行。任何一项缺失都应保持禁用；启用或授予会进入人工确认。
...
币种数据固定同步 Binance Spot 和 USD-M 的 USDT 市场。同步、订阅、补数、策略、回测和信号任务均从工作台节点进入。
...
1. 进入“工作流工作台”，新建工作流。
2. 从左侧节点库拖入开始、行情、策略、智能体、控制、通知或领域管理节点和结束节点。
3. 使用右侧结构化表单配置资源、Decimal、代码和选项；使用字段映射器连接数据端口，不填写 JSON 或状态路径。
4. 用 `flow` 边配置执行顺序和分支，用 `data` 边配置有类型的数据输入。
5. 使用工具栏校验并定位问题，保存后激活目标版本。
6. 对手工入口执行一次测试，在画布覆盖层和底部时间线查看完整轨迹。
...
- 每根 K 线或其他事件触发都会产生独立的有限执行记录；运行中可取消，失败可整次重跑，Worker 与人工任务会显示等待状态。
- 桌面端提供拖拽编辑；移动端只允许查看、回放、运行、取消、整次重跑和处理人工待办。
...
1. 在工作流中添加 `config.notification-channel` 节点并选择操作。
2. 在受控表单填写渠道类型、地址、凭据或收件配置。
3. 运行配置流程并按要求处理人工确认。
4. 使用通知节点发送非敏感测试消息，确认目标端收到后再用于正式流程。
...
1. 在工作流中添加 `config.ai-model` 节点。
2. 选择操作和供应商类型，在受控表单填写模型标识、Base URL 和凭据。
3. 运行配置流程，按需要启用模型并绑定智能体。

Base URL 是外部信任边界，只配置可信的 HTTPS 服务。模型凭据不得进入提示词、工作流状态、日志或版本库。

### 10.2 配置智能体

1. 在工作流中添加 `config.assistant` 节点，设置名称、系统提示词、头像和数据源约束。
2. 通过资源选择器绑定已启用模型并运行配置流程。
3. 在 `assistant.agent` 节点中选择该智能体，通过数据端口映射输入和输出。
4. 用非敏感测试数据验证结果，再用于正式工作流。
...
管理员使用 `admin.user` 节点创建、编辑或启停用户。所有修改均进入人工待办并要求重新认证；停用用户前确认其工作流、通知和量化任务的归属及后续处理方式。

### 11.2 角色

`admin.role` 与 `admin.permissions` 节点管理角色和动作权限，修改均要求人工确认与重新认证。采用最小权限原则：日常账号只授予实际需要的节点和动作，超级管理员只用于系统维护。

### 11.3 菜单

产品导航固定保留首页和工作台；旧菜单记录只作为权限载体且保持停用。隐藏菜单不等于 API 授权，真正的访问控制仍由后端 RBAC 在节点展示、激活和执行时完成。
