...
保存完整校验通过后才创建新修订。密钥输入只表示替换，留空保持原值；系统只显示是否已配置。同一节点实例的类型和版本不变时保留状态；删除已有状态的节点或修改其类型/版本时，先暂停工作流，再在保存确认框中明确重置对应状态。切换到历史修订时工作台只读。移动端不显示编辑控件，只提供工作流状态、最近活动和所选批次节点路径。
