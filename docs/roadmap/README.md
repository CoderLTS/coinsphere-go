...
> P0-P4 代码基线由自动化门禁覆盖，稳定能力以[架构概览](../architecture/overview.md)、[公共契约](../contracts/README.md)和[使用手册](../user-guide.md)为准；恢复、视觉验收、migration freeze 和 Paper 观察放行证据保存在 GitHub。
...
超级管理员可以从模板或最小空白图创建批处理工作流，在同一工作台完成连线、Schema 配置、显式保存、激活、观察和排障。每次保存产生不可变修订，每个手工或定时触发产生可重放运行。

### 建议交付切片

| 切片                        | 可直接验收的能力                                                                                |
| --------------------------- | ----------------------------------------------------------------------------------------------- |
| P1-A 工作流、修订与生命周期 | Workflow/Revision/Runtime 模型，模板创建，单触发器，save/activate/deactivate API 和原子修订切换 |
| P1-B Schema 工作台          | 左列表、中央 X6 画布、右检查器、JSON Schema/UI Schema 表单、结构化字段映射和 CEL 校验           |
| P1-C 运行与检查点           | 手工/定时触发、stream/compute 池、节点运行、稳定操作键、检查点、失败节点重试和协作取消          |
| P1-D 日志与制品             | 运行完整路径、节点多行日志、30 天默认保留和 SHA-256 内容寻址制品                                |
...
- 显式保存只在完整校验通过后创建修订并原子切换；已创建运行固定旧修订，失败保存不影响旧运行。
...
- 管理员能从工作台定位运行、节点尝试、耗时、错误和制品；停用后保留运行日志且不领取新运行。
...
工作流可以消费 CloudEvents 或由长运行 Trigger 持续产生事件。PostgreSQL 提供有界队列、分区顺序、背压和重启恢复；工作台按运行查询节点日志。Connector 与 AI 插件证明通用采集和动作能力。
...
| P2-D 日志与诊断重放         | 运行搜索、节点多行日志、固定输入/修订重放和副作用禁用                                   |
| P2-E Connector 与 AI        | HTTP/Webhook/WebSocket Connector、模型调用节点、节点本地密钥、结构化输入输出和插件结果页 |

### 退出条件

- `(source,id)` 去重，投递为至少一次；同分区非等待运行保持顺序，不同分区受限并发。
...
- 最终失败终止运行并发布标准失败事件；故障工作流可以据此通知，不要求每个节点维护错误边。
- 诊断重放固定原事件和修订，明确禁用 notification、human_action 和 paper 副作用。
- 运行与节点日志直接从 PostgreSQL 查询，不提供活动 WebSocket 或第二套活动事实源。
...
- 管理员可在工作台观察采集与回测运行，并从插件结果页查询行情、策略和回测结果。
...
| 运行、检查点和重放     | 基础执行 | batch        | event/stream/等待 | compute 回测 | Paper 副作用与恢复              |
