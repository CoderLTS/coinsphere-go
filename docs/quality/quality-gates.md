...
- 数据库：SQLite 与 PostgreSQL 的空库升级、旧版本保数升级、回滚重放、重复 Up 和失败原子性契约；Worker 任务表必须覆盖七态、尝试次数、租约唯一性及非空表拒绝 Down；Outbox 必须覆盖五态、活跃租约字段一致性、尝试次数、可用/终态时间、必要索引、非空表拒绝 Down，以及双认领者争抢、原子批量上限、批量失败全回滚、调用方事务回滚、续租、失败退避、过期恢复、尝试耗尽、死信告警争抢和旧 token fencing；工作流服务必须在双方言运行同一套并发版本、并发激活、失败回滚和一致快照契约。
...
- Outbox schema、双方言存储契约与 dispatcher 已覆盖事务生产、存储层原子批量认领、dispatcher 即时逐条认领、续租、数据库时间 fencing、指数退避、订阅失败重试、过期恢复、尝试耗尽和死信日志告警。告警以 `alerted_at` 提供并发下 at-most-once 去重，不是可靠外部通知；全部 Outbox 日志不得输出事件 payload、metadata、环境值、凭据、Owner、token 或异常正文。
- A1-4 已以双方言服务契约覆盖工作流版本与运行态事务：并发版本唯一且单调递增，激活/停用和入口替换原子提交，失败不改变原 active 版本及入口，读取不混合新旧快照。本阶段复用现有约束，不新增 migration；`AutoMigrate` 仍由 A1-10 独立移除。
