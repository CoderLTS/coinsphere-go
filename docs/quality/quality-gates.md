...
- 数据库：SQLite 与 PostgreSQL 的空库升级、旧版本保数升级、回滚重放、重复 Up 和失败原子性契约；Worker 任务表必须覆盖七态、尝试次数、租约唯一性及非空表拒绝 Down；Outbox 必须覆盖五态、活跃租约字段一致性、尝试次数、可用/终态时间、必要索引、非空表拒绝 Down，以及双认领者争抢、原子批量上限、批量失败全回滚、调用方事务回滚、续租、失败退避、过期恢复、尝试耗尽、死信告警争抢和旧 token fencing。
