...
持续账户、订单和仓位快照对账、可证明归属的外部主调仓恢复，以及逐笔成交、真实手续费和资金费的权威追加对账已交付。`GET /api/v1/trading/overview` 只向账户 Owner 返回最近 100 条 Testnet 权威事实，金额和数量保持十进制字符串，交易所 ID 保持字符串；该只读审计视图不触发对账或外部请求。生产继续保持 Testnet 私有能力关闭；后续晋级遵守 [ADR-0010](../architecture/decisions/0010-execution-risk-events.md) 并由用户手工放行。
