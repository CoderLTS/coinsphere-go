...
Codex 可以推送 `codex/*` 分支并创建 Draft PR，但不能合并。仓库为私有且当前套餐不提供完整 Branch Protection/Environment required reviewers，因此用户审查、手工合并、最新 `main` 校验和最终只读复审是现阶段补偿控制。
...
- 被新路线替代的旧 A2-A8 跟踪 Issue 关闭但不删除，并在关闭评论中链接替代 Issue；已经完成的实现 Issue 和 CI 证据保留为历史事实。
...
- 生产发布仅允许用户从最新 `main` 手工触发。进入 Live 前按发布 Runbook 分离构建与固定部署器。
