...
- `00024` 是 Graph v3 的破坏性基线切换，只允许在已确认为空且可恢复的工作流数据库上执行；Down 永远拒绝，不能把旧 Graph v1/v2 数据库原地回滚到新模型。
