...
当前部署包含 Binance Spot 与 USD-M 单向合约的市价单、私有 User Data Stream 和 REST 对账实现，但真实交易默认关闭。发布、CI、Codex 和普通工作流不得提供真实交易凭据、自动打开交易开关或代替账户级人工放行；API Key 必须禁用提现权限，并通过 `SecretReader` 在运行时读取。

启用真实交易前必须逐账户完成发布确认，并配置单笔名义价值、单品种持仓价值、日亏损、每日订单数、滑点和 Quote 年龄上限。任一门禁缺失时保持 Paper 模式；回滚应用版本不会自动撤单或清除交易事实，须先停用账户发布并通过 Binance REST 核对未完成订单和持仓。
