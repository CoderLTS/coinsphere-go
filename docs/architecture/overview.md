...
活动 Testnet 账户持续轮询权威账户快照。Reconciler 只接受本地确定性订单能解释的开放订单和仓位，缺失的本地活动订单按原 `clientOrderId` 查询恢复；在 `matched` 快照后，已管理成交订单的逐笔成交、手续费和 USD-M 资金费追加到独立的 `testnet_trade_facts` 事实表，并由交易所 ID 幂等去重。未知外部订单、未归属仓位、订单形状漂移、累计成交差异或查询未知都会暂停账户。账户与凭据版本锁、订单更新时间栅栏和快照观察时间栅栏共同阻止旧结果覆盖新状态；持续对账不自动恢复账户、不修改外部订单，未知外部订单归属恢复仍留待后续阶段。
