...
Spot Live 复用确定性订单、保护单、权威对账和硬风控管线，但运行在独立的 `live` 环境范围。manual 与 auto 均默认关闭且只允许 Spot；auto 必须在 manual 已启用、账户已手工恢复后，同时具备管理员授权、Owner 自动交易放行、完整风控和匹配对账。两个 Executor 以 `mode` 隔离领取与恢复意图。每次暂停都会清除 Owner 的 manual/auto 放行并关闭自动化，保护单和紧急平仓仍可继续；USD-M Live 没有运行时路径。当前只完成 Paper 与离线契约验证，不把它视为 Binance 环境晋级证据。
