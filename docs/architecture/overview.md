...
活动 Testnet 账户持续轮询权威账户快照。Reconciler 只接受本地确定性订单能解释的开放订单和仓位，缺失的本地活动订单按原 `clientOrderId` 查询恢复；未知外部订单、未归属仓位、订单形状漂移或查询未知都会暂停账户。账户与凭据版本锁、订单更新时间栅栏和快照观察时间栅栏共同阻止旧结果覆盖新状态；持续对账不自动恢复账户、不修改外部订单，也不替代后续逐笔成交、真实手续费和资金费对账。
