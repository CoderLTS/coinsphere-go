...
- 用户在一个工作台内创建、连接、配置、激活、停用、搜索日志和排障，不需要理解内部入口或原始运行输入。
- 一个节点既声明配置界面，也声明输入、输出、执行语义和结果展示，使工作流核心不包含量化、AI 或通知的私有协议。
- 持续运行的工作流仍由有界、可重放的事件 Run 组成，任何失败都能定位到固定修订、事件引用、节点尝试和多行日志。
...
| 用户       | 首页             | 工作流                                               | 结果页                     | 管理功能                             |
| ---------- | ---------------- | ---------------------------------------------------- | -------------------------- | ------------------------------------ |
| 超级管理员 | 系统与工作流健康 | 创建、编辑、保存、激活、停用、查日志、重试、取消、重放 | 全部管理视图和已共享视图   | 用户、角色、菜单、插件状态、系统设置 |
...
- 工作流定义列表只显示名称、版本、激活状态、创建时间和操作，每行日志按钮进入该工作流的 Run 搜索。
- Run 详情复用 X6 工作流画布，节点检查器显示真实节点尝试、输入输出摘要和多行日志，不生成伪尝试或空边流转。

创建工作流时先选择场景模板。官方首批模板为共享行情采集、策略回测、Paper 策略和故障通知；“空白高级工作流”仍创建一份可校验的最小图，由一个手工触发器连接结束节点。

移动端提供系统监控、结果页、审批和 Run 详情，不提供画布编辑。普通用户不会看到源工作流名称、ID、拓扑或跳转入口。
...
| 模块       | 拥有的状态与职责                                                       | 不负责                               |
| ---------- | ---------------------------------------------------------------------- | ------------------------------------ |
| Web        | 工作台交互、Schema 表单、Run 日志视图、插件结果组件、RBAC 路由         | 业务幂等、密钥保存、运行调度         |
| API/RBAC   | 认证、权限、结果范围解析、请求校验、审计                               | 插件领域计算、逐事件编排             |
| 工作流核心 | 图校验、修订、生命周期、事件入队、Run、检查点、重试、人工等待、节点日志 | Binance 私有协议、策略公式、通知协议 |
...
| 实体               | 关键字段                                            | 不变量                                   |
| ------------------ | --------------------------------------------------- | ---------------------------------------- |
| `Workflow`         | ID、名称、模式、状态、活动修订 ID、主触发器、保留期 | 一个主触发器；状态仅 inactive/active/error |
| `WorkflowRevision` | 修订号、不可变图、节点类型版本、创建者、创建时间    | 保存后不更新；Run 始终固定一个修订       |
| `WorkflowRuntime`  | workflow ID、并发、积压和定时调度字段               | 不保存活动游标或重复健康状态             |
| `EventRecord`      | CloudEvent、分区键、入队时间、投递状态              | `(source,id)` 唯一；先持久化后投递       |
| `WorkflowRun`      | 事件引用、修订、状态、分区、租约、诊断关联、结果摘要 | 每次触发一个有界 Run；终态不可改写       |
| `WorkflowRunNode`  | Run、nodeInstanceId、轮次、尝试号、状态、摘要、耗时 | 每次尝试只有一个终态                     |
| `WorkflowNodeLog`  | RunNode、UTC 时间、级别、消息、受限字段             | 多行追加；不保存密钥或原始载荷           |
| `RunCheckpoint`    | 成功输出、状态和制品引用                            | 成功节点提交后原子可见                   |
...
```mermaid
stateDiagram-v2
    [*] --> inactive: 从模板创建有效初始修订
    inactive --> active: activate
    active --> inactive: deactivate
    active --> error: Trigger 异常退出
    error --> inactive: deactivate
```

- `activate` 只在存在完整有效的活动修订、所需密钥和状态均可用时成功。
- `deactivate` 停止调度、断开长连接触发器并停止领取新 Run；当前节点结束后保存检查点，已持久化事件和 Run 保留到恢复。
- Trigger 异常退出进入 `error`，不会无界重启。管理员必须先停用恢复为 `inactive`，检查配置后再显式激活。
...
节点取消通过 `context.Context` 协作完成。取消请求发出后不再调度后续节点，当前 Go 处理器返回时 Run 进入 `cancelled`。无法响应取消的可信插件不能在主进程内被强制终止；出现实际隔离需求后再引入独立 Go Worker。

人工任务使用 `pending/approved/rejected/expired/superseded`。进入等待后，核心保存上下文并释放执行池和分区占用；恢复结果作为节点的类型化输出继续执行。新信号可以按节点配置的业务键把旧 `pending` 任务置为 `superseded`，旧 Run 随后以“已取代”结果继续，而不是作为系统失败处理。
...
运行中的 Run 继续使用创建时的修订和密钥绑定；后续事件使用新修订。保存失败或事务回滚时，旧修订和 Run 不受影响。
...
- PostgreSQL 持久队列按工作流、主触发器和分区维护 Run；同一分区任一时刻最多有一个非等待 Run 推进，不同分区可以并发。
- 人工或持久等待保存检查点后释放分区，因此后续同分区事件可以继续并产生取代动作；恢复仍使用原 Run 修订。
...
- 单实例故障不会丢失已提交事件。重启后租约超时的 Run 重新进入可领取状态，并从最后成功检查点继续，原节点尝试标记失败并写入恢复日志。
...
```text
sha256(runId + ":" + nodeInstanceId + ":" + loopIteration)
```

成功输出、制品引用和节点状态在同一事务内提交为检查点。后续失败只重试失败节点，不重新运行已成功节点。节点级重试策略声明最大次数、退避和可重试错误类别；耗尽后 Run 进入 `failed`，核心发布 `io.coinsphere.workflow.run.failed` CloudEvent。
...
诊断重放创建关联原 Run 的新 Run，固定原事件、原修订和原始制品。节点描述符按 `none/notification/human_action/paper` 标记副作用；后三类在诊断重放中不执行真实副作用，而复用原检查点。诊断重放不能成为补发通知或重做 Paper 成交的入口。

工作流列表提供日志入口。日志页默认最近 24 小时，可按 UTC 时间、Run 状态、触发类型和关键词查询；详情返回事件摘要、全部真实 RunNode 尝试、节点多行日志、脱敏输入输出摘要、结果和制品。Activity 表、Activity API 和活动 WebSocket 不再存在。

### 5.5 保留、制品和容量

- WorkflowRun、RunNode、节点日志、RunCheckpoint 和制品默认保留 30 天，管理员可按工作流调整；审计和插件金融事实遵循各自更长的领域策略。
...
| `plugin upgrade <dir> --force-major` | 当前拒绝；重大升级需先停用引用工作流并通过独立兼容设计处理                         |
...
| `/api/v1/workflows/{id}/lifecycle` | activate、deactivate              | 超级管理员               |
| `/api/v1/workflows/{id}/runs`      | 手工触发、搜索运行日志            | 超级管理员               |
| `/api/v1/workflow-runs/{id}`       | 节点路径、日志、重试和协作取消    | 超级管理员               |
...
Paper 场景创建两个工作流：平台级共享行情采集流，以及策略到 Paper 的用户流。回测使用单独模板和 compute 池，每次触发建立独立 Run。
...
系统监控首页继续展示应用、数据库、执行池、事件积压、失败 Run、长时间等待、插件版本和 migration 状态。工作流日志页展示单工作流最近 Run、节点耗时和错误。只有状态拥有模块记录结构化日志，避免调用链重复告警。
...
| 范围       | 必须证明的场景                                                                                           |
| ---------- | -------------------------------------------------------------------------------------------------------- |
| 图与修订   | 单触发器、Schema、字段类型、CEL、无任意环、Loop 限制；保存原子切换；旧 Run 继续旧修订；失败保存不影响运行 |
| 配置与状态 | 密钥不进入 JSON/日志/导出；复制不带密钥和状态；兼容修改保留状态；破坏性修改要求停用和重置                |
| 事件与 Run | CloudEvents 校验、每 K 线一 Run、分区顺序、跨分区并发、背压、检查点续跑、重试耗尽和进程重启恢复          |
| 副作用     | 稳定操作键、崩溃重试不重复、人工等待释放容量、信号取代、诊断重放不通知/审批/Paper                        |
| 运行日志   | UTC/状态/关键词搜索、Run 完整节点路径、多行日志脱敏、30 天保留和大型制品哈希一致                         |
