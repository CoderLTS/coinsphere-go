...
async function handleRouteGuard(
  ...
  // 5. 处理已匹配的路由
  if (to.matched.length > 0) {
    // 每次正常导航前都清理一次失效工作标签，避免旧缓存残留 404 标签。
  ...
  if (menuStore.menuList.length > 0 && !isStaticRoute(to.path)) {
...
/**
 * 拉取菜单国际化字典，并合并到运行时 i18n 实例。
 */
async function refreshMenuI18nDict(): Promise<void> {
  const dict = await fetchGetMenuI18nDict()
  mergeRuntimeMenuI18nDict(dict)
}
