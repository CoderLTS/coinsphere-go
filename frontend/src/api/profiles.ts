...
const profileBase = (pluginId: string, type?: string) =>
  `/api/v1/plugins/${encodeURIComponent(pluginId)}/profiles${type ? `/${encodeURIComponent(type)}` : ''}`
...
export const fetchProfile = (pluginId: string, type: string, profileId: string) =>
  request.get<{ item: ProfileRecord }>({
    url: `${profileBase(pluginId, type)}/${encodeURIComponent(profileId)}`
  })
...
export const copyProfile = (pluginId: string, type: string, profileId: string, name?: string) =>
  request.post<ProfileRecord>({
    url: `${profileBase(pluginId, type)}/${encodeURIComponent(profileId)}/copy`,
    data: name ? { name } : {}
  })
...
export const publishProfile = (
  ...
  request.post<ProfileRecord>({
    url: `${profileBase(pluginId, type)}/${encodeURIComponent(profileId)}/publish`,
    data: { version }
  })

export const disableProfile = (pluginId: string, type: string, profileId: string) =>
  request.post<ProfileRecord>({
    url: `${profileBase(pluginId, type)}/${encodeURIComponent(profileId)}/disable`
  })
