    ...
    const validateWorktabs = (routerInstance: Router): void => {
      try {
        ...
        const isResolvedRouteUsable = (tab: Partial<WorkTab>, route: ReturnType<Router['resolve']>): boolean => {
          ...
          if (matchedRoute.name === 'Exception404' && tab.path !== '/404') {
            return false
          }

          if (matchedRoute.meta?.isHideTab) {
            return false
          }
        ...
        const isTabRouteValid = (tab: Partial<WorkTab>): boolean => {
          try {
            if (tab.name) {
              const namedRoute = routes.find((r) => r.name === tab.name)
              if (namedRoute) {
                if (namedRoute.name === 'Exception404' && tab.path !== '/404') {
                  return false
                }
                return !namedRoute.meta?.isHideTab
              }
            }
            if (tab.path) {
              ...
              return resolved.matched.length > 0 && isResolvedRouteUsable(tab, resolved)
            }
            return false
          } catch {
            return false
        ...
        const resolveTabRoute = (tab: Partial<WorkTab>) => {
          ...
          if (!isResolvedRouteUsable(tab, resolved)) {
            return null
          }

          return resolved.matched[resolved.matched.length - 1] || null
        }

        // Normalize persisted tab metadata from the current route table so old
        // cached titles such as "????" are replaced after a deployment.
        const normalizeTab = (tab: WorkTab): WorkTab => {
          ...
          return {
            ...
            icon: (route.meta?.icon as string) || tab.icon,
            fixedTab: (route.meta?.fixedTab as boolean) ?? tab.fixedTab,
            keepAlive: (route.meta?.keepAlive as boolean) ?? tab.keepAlive
          }
        }

        // 过滤出有效的标签页
        const validTabs = opened.value.filter((tab) => isTabRouteValid(tab)).map(normalizeTab)
        ...
        if (!isCurrentValid && validTabs.length > 0) {
          ...
          const normalizedCurrent =
            validTabs.find((tab) => tab.path === current.value.path) ||
            validTabs.find((tab) => tab.name === current.value.name)

          if (normalizedCurrent) {
            current.value = normalizedCurrent
          }
        }
      } catch (error) {
