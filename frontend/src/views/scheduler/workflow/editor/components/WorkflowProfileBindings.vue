      ...
      <ElFormItem :label="slot.title" :required="slot.required">
        ...
        >
          ...
          >
            ...
            <small class="profile-bindings__option-summary">{{ profile.summary || profile.type }}</small>
          </ElOption>
        </ElSelect>
        <div v-if="selectedProfile(slot.key)" class="profile-bindings__summary">
          {{ selectedProfile(slot.key)!.summary || '已发布 Profile' }} · {{ selectedProfile(slot.key)!.latestPublishedVersion || selectedProfile(slot.key)!.version }}
        </div>
      </ElFormItem>
    </div>
    <ElAlert v-if="loadingError" type="warning" :closable="false" title="Profile 列表暂时不可用，可稍后刷新页面重试" />
  </section>
</template>
...
const profileVersion = (profile: ProfileRecord) => profile.version || profile.latestPublishedVersion || ''
const profileKey = (profile: ProfileRecord) => `${profile.pluginId}:${profile.id}:${profileVersion(profile)}:${profile.type}`
const profileLabel = (profile: ProfileRecord) => `${profile.name} · ${profile.summary || profile.type} · ${profileVersion(profile) || '未发布'}`
const profilesFor = (slot: { profileTypes: string[] }) => profiles.value.flatMap(profile => {
  ...
  if (published.length) return published.map(version => ({ ...profile, version: version.version, latestPublishedVersion: version.version, config: version.config }))
  return profile.latestPublishedVersion || profile.version ? [profile] : []
})
const selectedProfile = (key: string) => {
  const ref = props.node.profileBindings?.[key]
  return profilesFor({ profileTypes: ref?.type ? [ref.type] : [] }).find(p => p.pluginId === ref?.pluginId && p.id === ref?.profileId && p.type === ref?.type && profileVersion(p) === ref?.version)
}
const select = (key: string, value: string | undefined) => {
  ...
  if (!value) delete next[key]
    ...
    const found = profilesFor({ profileTypes: [type.join(':')] }).find(p => p.pluginId === pluginId && p.id === profileId && p.type === type.join(':') && profileVersion(p) === version)
...
const load = async () => {
  ...
  try {
    ...
    const results = await Promise.all(owners.flatMap(owner => types.map(type => fetchProfiles(owner, type).catch(() => ({ items: [] as ProfileRecord[] })))))
    const unique = new Map<string, ProfileRecord>()
    results.flatMap(result => result.items).forEach(profile => unique.set(`${profile.pluginId}:${profile.id}:${profile.latestPublishedVersion || profile.version}`, profile))
...
<style scoped>
.profile-bindings { border-top: 1px solid var(--el-border-color-lighter); padding-top: 4px; }
.profile-bindings__title { margin: 18px 0 12px; font-size: 13px; font-weight: 600; }
.profile-bindings__summary { margin-top: 4px; color: var(--el-text-color-secondary); font-size: 12px; line-height: 1.5; }
.profile-bindings__option-summary { display: block; color: var(--el-text-color-secondary); font-size: 11px; }
</style>
