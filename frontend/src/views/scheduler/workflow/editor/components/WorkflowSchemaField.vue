...
-->
<template>
  <ElDatePicker v-if="field.format === 'date-time'" :model-value="value ? new Date(value) : undefined" type="datetime" placeholder="选择日期与时间（本地显示，UTC 保存）" @update:model-value="next => $emit('update', next ? new Date(next).toISOString() : '')" />
  <ElSelect
