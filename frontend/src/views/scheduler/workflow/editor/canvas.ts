...
export function canvasNode(
  ...
  return {
    ...
    attrs: {
      body: { fill: 'var(--el-bg-color)', stroke: color, strokeWidth: 2, rx: 8, ry: 8 },
      label: {
        fill: 'var(--el-text-color-primary)',
        fontSize: 13,
        textWrap: { width: 186, height: 55, ellipsis: true }
      }
    },
    ports: {
      groups: {
        in: {
          position: 'left',
          attrs: { circle: { r: 5, magnet: 'passive', stroke: color, fill: 'var(--el-bg-color)' } }
        },
        out: {
          position: 'right',
          attrs: {
            circle: { r: 5, magnet: true, stroke: color, fill: 'var(--el-bg-color)' },
            text: { fill: 'var(--el-text-color-secondary)', fontSize: 10 }
          },
...
export function canvasEdges(graph: WorkflowGraph): EdgeMetadata[] {
...
export function schemaDefaults(schema: Record<string, any>): Record<string, any> {
  return Object.fromEntries(
    Object.entries(schema.properties || {}).flatMap(([key, raw]) => {
  ...
  function visit(node: WorkflowGraphNode, schema: Record<string, any>, path: string[]) {
    ...
    if (path.length)
      fields.push({
        ...
        path,
        label: `${node.label || definitions.find((d) => d.type === node.nodeType)?.title || node.nodeType} · ${path.join('.')}`,
        schema
      })
    for (const [key, property] of Object.entries(schema.properties || {}))
      visit(node, property as Record<string, any>, [...path, key])
  }
  for (const node of graph.nodes.filter((n) => upstream.has(n.nodeInstanceId)))
    visit(node, definitions.find((d) => d.type === node.nodeType)?.outputSchema || {}, [])
  return fields
}
