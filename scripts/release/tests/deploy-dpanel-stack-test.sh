...
if ! grep -Fq 'stop coinsphere-backend coinsphere-web' "$DEPLOY_DOCKER_LOG" \
  || ! grep -Fq 'up -d --no-deps --wait --wait-timeout 180 coinsphere-backend coinsphere-web' "$DEPLOY_DOCKER_LOG" \
  || ! grep -Fq "run --rm --network dpanel_stack --env-file $STACK_ROOT/secrets/coinsphere-runtime.env $REGISTRY/coinsphere/backend@sha256:$BACKEND_DIGEST /app/coinsphere-migrate" "$DEPLOY_DOCKER_LOG"; then
