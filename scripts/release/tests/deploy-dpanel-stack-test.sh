...
if ! grep -Fq 'stop coinsphere-backend coinsphere-web coinsphere-worker coinsphere-worker-backtest' "$DEPLOY_DOCKER_LOG" \
  || ! grep -Fq 'up -d --no-deps --wait --wait-timeout 180 coinsphere-backend coinsphere-web coinsphere-worker coinsphere-worker-backtest' "$DEPLOY_DOCKER_LOG" \
