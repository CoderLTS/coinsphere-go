...
func registerPrivateAccountStream(registrar sdk.Registrar, runtime *binanceRuntime) error {
	return registrar.Trigger(withNodeMeta(sdk.NodeDescriptor{
		...
		ConfigSchema: json.RawMessage(`{"$schema":"https://json-schema.org/draft/2020-12/schema","type":"object","properties":{"account":{"type":"string","title":"Account","pattern":"^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$"},"market":{"type":"string","title":"Market","enum":["spot","usdm"],"default":"spot"},"proxyId":{"type":"integer","title":"Proxy","minimum":0,"default":0,"x-coinsphere-proxy":true},"reconciliationSeconds":{"type":"integer","title":"REST reconciliation interval","minimum":30,"maximum":3600,"default":60},"apiKey":{"type":"string","title":"API key","x-coinsphere-secret":true},"apiSecret":{"type":"string","title":"API secret","x-coinsphere-secret":true}},"required":["account","market","reconciliationSeconds","apiKey","apiSecret"],"additionalProperties":false}`),
