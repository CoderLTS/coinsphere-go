...
-- The M3 table names are retained because migration history is frozen. From
-- this migration onward they store projections for both isolated private
-- environments; the owning account remains the authoritative environment.
...
-- The remaining functions restore the M3 Testnet-only predicates. Their
-- order/protection and append-only fact shapes are otherwise unchanged.
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION validate_testnet_order_binding() RETURNS TRIGGER
LANGUAGE plpgsql AS $$
DECLARE
