...
`).Scan(&versionTables); err != nil {
	...
	results, err = runner.Down(context.Background(), 17)
	if err != nil {
	...
	for index, result := range results {
		if result.Version != int64(17-index) || result.Direction != "down" {
			t.Fatalf("migration rollback results = %#v", results)
		}
	}
	if err := runner.ValidateCurrent(context.Background()); err == nil {
...
func TestPostgresBaselineDownRejectsData(t *testing.T) {
	...
	if versionErr != nil || current != 1 || latest != 17 {
...
func TestObservabilityDownRejectsAuditData(t *testing.T) {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 2 || latest != 17 {
...
func TestA2MarketContractDownRejectsData(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			current, latest, versionErr := runner.Versions(context.Background())
			if versionErr != nil || current != 3 || latest != 17 {
...
`, ownerID, instrumentID); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 4 || latest != 17 {
...
`); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 5 || latest != 17 {
...
`, backtestID); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 6 || latest != 17 {
...
`, signalID, ownerID, externalChannelID); err == nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 8 || latest != 17 {
...
`, signalID); err != nil {
	...
	current, latest, versionErr = runner.Versions(context.Background())
	if versionErr != nil || current != 7 || latest != 17 {
...
`, eventID, accountID, instrumentID); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 10 || latest != 17 {
...
`, testnetAccountID); err == nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 11 || latest != 17 {
...
`, accountID); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 15 || latest != 17 {
...
`, accountID); err == nil {
	...
	current, latest, versionErr = runner.Versions(context.Background())
	if versionErr != nil || current != 12 || latest != 17 {
...
`, accountID); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 13 || latest != 17 {
...
`, instanceID, ownerID, versionID); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 14 || latest != 17 {
...
`, accountID, instrumentID); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 16 || latest != 17 {
