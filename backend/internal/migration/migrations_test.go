...
`).Scan(&versionTables); err != nil {
	...
	results, err = runner.Down(context.Background(), 12)
	if err != nil {
	...
	for index, result := range results {
		if result.Version != int64(12-index) || result.Direction != "down" {
			t.Fatalf("migration rollback results = %#v", results)
		}
	}
	if err := runner.ValidateCurrent(context.Background()); err == nil {
...
func TestPostgresBaselineDownRejectsData(t *testing.T) {
	...
	if versionErr != nil || current != 1 || latest != 12 {
...
func TestObservabilityDownRejectsAuditData(t *testing.T) {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 2 || latest != 12 {
...
func TestValidateCurrentRejectsDatabaseAhead(t *testing.T) {
	...
	if _, err := database.Exec(`INSERT INTO schema_migrations (version_id, is_applied) VALUES (13, TRUE)`); err != nil {
...
func TestA2MarketContractDownRejectsData(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			current, latest, versionErr := runner.Versions(context.Background())
			if versionErr != nil || current != 3 || latest != 12 {
...
`, ownerID, instrumentID); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 4 || latest != 12 {
...
`); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 5 || latest != 12 {
...
`, backtestID); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 6 || latest != 12 {
...
`, signalID, ownerID, externalChannelID); err == nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 8 || latest != 12 {
...
`, signalID); err != nil {
	...
	current, latest, versionErr = runner.Versions(context.Background())
	if versionErr != nil || current != 7 || latest != 12 {
...
`, eventID, accountID, instrumentID); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 10 || latest != 12 {
...
`, testnetAccountID); err == nil {
	...
	if _, err := runner.Down(context.Background(), 1); err != nil {
		t.Fatalf("roll back empty Testnet reconciliation migration: %v", err)
	}
	if _, err := runner.Down(context.Background(), 1); err == nil {
		t.Fatal("Testnet credential rollback removed persistent data")
	}
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 11 || latest != 12 {
...
`, accountID); err == nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 12 || latest != 12 {
...
func assertCurrentTables(t *testing.T, database *sql.DB) {
	...
	want := []string{
		...
		"task_definition_configs", "testnet_balances", "testnet_open_orders", "testnet_positions", "testnet_reconciliations",
