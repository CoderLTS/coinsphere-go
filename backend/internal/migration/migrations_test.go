...
`).Scan(&versionTables); err != nil {
	...
	results, err = runner.Down(context.Background(), 21)
	if err != nil {
	...
	for index, result := range results {
		if result.Version != int64(21-index) || result.Direction != "down" {
			t.Fatalf("migration rollback results = %#v", results)
		}
	}
	if err := runner.ValidateCurrent(context.Background()); err == nil {
...
func TestPostgresBaselineDownRejectsData(t *testing.T) {
	...
	if versionErr != nil || current != 1 || latest != 21 {
...
func TestObservabilityDownRejectsAuditData(t *testing.T) {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 2 || latest != 21 {
...
func TestPostgresBaselineDownSeesConcurrentCommit(t *testing.T) {
	...
	if _, err := runner.Down(context.Background(), 20); err != nil {
...
func TestValidateCurrentRejectsDatabaseAhead(t *testing.T) {
	...
	_, latest, err := runner.Versions(context.Background())
	if err != nil {
		t.Fatalf("read migration versions: %v", err)
	}
	if _, err := database.Exec(`INSERT INTO schema_migrations (version_id, is_applied) VALUES ($1, TRUE)`, latest+1); err != nil {
...
func TestA2MarketContractDownRejectsData(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			current, latest, versionErr := runner.Versions(context.Background())
			if versionErr != nil || current != 3 || latest != 21 {
...
`, ownerID, instrumentID); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 4 || latest != 21 {
...
`); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 5 || latest != 21 {
...
`, backtestID); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 6 || latest != 21 {
...
`, signalID, ownerID, externalChannelID); err == nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 8 || latest != 21 {
...
`, signalID); err != nil {
	...
	current, latest, versionErr = runner.Versions(context.Background())
	if versionErr != nil || current != 7 || latest != 21 {
...
INSERT INTO trading_accounts (
    id, owner_user_id, name, market_type, environment, initial_balance, paper_fee_rate,
    automation_enabled, creation_idempotency_record_id
) VALUES ($1, $2, 'USD-M Live auto forbidden', 'usd_m', 'live', 10000, 0.001, TRUE, $3)
`, invalidAccountID, ownerID, invalidRecordID); err == nil {
		t.Fatal("trading account schema accepted USD-M Live auto state")
	}
	if _, err := database.Exec(`
...
`, eventID, accountID, instrumentID); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 10 || latest != 21 {
...
`, testnetAccountID); err == nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 11 || latest != 21 {
...
`, accountID); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 15 || latest != 21 {
...
`, accountID); err == nil {
	...
	current, latest, versionErr = runner.Versions(context.Background())
	if versionErr != nil || current != 12 || latest != 21 {
...
`, accountID); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 13 || latest != 21 {
...
`, instanceID, ownerID, versionID); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 14 || latest != 21 {
...
`, accountID, instrumentID); err != nil {
	...
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 16 || latest != 21 {
...
func TestM4SpotLiveManualConstraintsAndDownGuard(t *testing.T) {
	database := openPostgresSchema(t)
	runner, err := New(database)
	if err != nil {
		t.Fatalf("create migration runner: %v", err)
	...
	const accountID = "019de000-0000-7000-8000-000000000001"
	var ownerID, recordID int64
	if err := database.QueryRow(`INSERT INTO users (username) VALUES ('spot-live-owner') RETURNING id`).Scan(&ownerID); err != nil {
		t.Fatalf("insert Live owner: %v", err)
	}
	if err := database.QueryRow(`
INSERT INTO idempotency_records (user_id, scope, key_hash, request_hash, expires_at, created_at)
VALUES ($1, 'trading-account:create', repeat('e', 64), repeat('f', 64),
        CURRENT_TIMESTAMP + INTERVAL '1 day', CURRENT_TIMESTAMP)
RETURNING id
`, ownerID).Scan(&recordID); err != nil {
		t.Fatalf("insert Live idempotency record: %v", err)
	}
	if _, err := database.Exec(`
INSERT INTO trading_accounts (
    id, owner_user_id, name, market_type, environment, initial_balance,
    paper_fee_rate, creation_idempotency_record_id
) VALUES ($1, $2, 'Spot Live manual', 'spot', 'live', 1000, 0.001, $3)
`, accountID, ownerID, recordID); err != nil {
		t.Fatalf("insert Spot Live account: %v", err)
	}
	if _, err := database.Exec(`
UPDATE trading_accounts SET status = 'active', pause_reason = '' WHERE id = $1
`, accountID); err == nil {
		t.Fatal("Live account became active without a manual release record")
	}
	if _, err := database.Exec(`
UPDATE trading_accounts
SET status = 'active', pause_reason = '', manual_authorized_at = CURRENT_TIMESTAMP,
    manual_authorized_by_user_id = $2
WHERE id = $1
...
INSERT INTO trading_account_credentials (
    id, account_id, owner_user_id, api_key_ciphertext, api_secret_ciphertext,
    withdrawal_disabled, ip_whitelist_configured
) VALUES ('019de000-0000-7000-8000-000000000002', $1, $2,
          'ciphertext-key', 'ciphertext-secret', TRUE, TRUE)
`, accountID, ownerID); err != nil {
		t.Fatalf("insert Spot Live credential: %v", err)
	}
	if _, err := database.Exec(`UPDATE trading_accounts SET market_type = 'usd_m' WHERE id = $1`, accountID); err == nil {
		t.Fatal("Spot Live account changed to USD-M")
	}
	assertPostgresIndexes(t, database, []string{
		"uq_trading_intents_live_account_active", "ix_trading_intents_live_runnable",
	})
	if _, err := runner.Down(context.Background(), 2); err != nil {
		t.Fatalf("roll back empty USD-M Live manual migration: %v", err)
	}
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 19 || latest != 21 {
		t.Fatalf("USD-M Live manual rollback versions = current:%d latest:%d err:%v", current, latest, versionErr)
	}
...
`, accountID); err != nil {
	...
	current, latest, versionErr = runner.Versions(context.Background())
	if versionErr != nil || current != 18 || latest != 21 {
		t.Fatalf("Spot Live rollback versions = current:%d latest:%d err:%v", current, latest, versionErr)
	}
}

func TestM4USDMLiveManualConstraintsAndDownGuard(t *testing.T) {
	database := openPostgresSchema(t)
	runner, err := New(database)
	if err != nil {
		t.Fatalf("create migration runner: %v", err)
	...
	const accountID = "019de200-0000-7000-8000-000000000001"
	var ownerID, recordID int64
	if err := database.QueryRow(`INSERT INTO users (username) VALUES ('usdm-live-owner') RETURNING id`).Scan(&ownerID); err != nil {
		t.Fatalf("insert USD-M Live owner: %v", err)
	}
	if err := database.QueryRow(`
INSERT INTO idempotency_records (user_id, scope, key_hash, request_hash, expires_at, created_at)
VALUES ($1, 'trading-account:create', repeat('1', 64), repeat('2', 64),
...
RETURNING id
`, ownerID).Scan(&recordID); err != nil {
		t.Fatalf("insert USD-M Live idempotency record: %v", err)
	}
	if _, err := database.Exec(`
INSERT INTO trading_accounts (
    id, owner_user_id, name, market_type, environment, initial_balance,
    paper_fee_rate, leverage, creation_idempotency_record_id
) VALUES ($1, $2, 'USD-M Live manual', 'usd_m', 'live', 1000, 0.001, 2, $3)
`, accountID, ownerID, recordID); err != nil {
		t.Fatalf("insert USD-M Live account: %v", err)
	}
	if _, err := database.Exec(`
UPDATE trading_accounts SET status = 'active', pause_reason = '' WHERE id = $1
`, accountID); err == nil {
		t.Fatal("USD-M Live account became active without a manual release record")
	}
	if _, err := database.Exec(`
UPDATE trading_accounts
SET status = 'active', pause_reason = '', manual_authorized_at = CURRENT_TIMESTAMP,
...
WHERE id = $1
`, accountID, ownerID); err != nil {
		t.Fatalf("activate manually released USD-M Live account: %v", err)
	}
	if _, err := database.Exec(`
INSERT INTO trading_account_credentials (
    id, account_id, owner_user_id, api_key_ciphertext, api_secret_ciphertext,
    withdrawal_disabled, ip_whitelist_configured
) VALUES ('019de200-0000-7000-8000-000000000002', $1, $2,
          'ciphertext-key', 'ciphertext-secret', TRUE, TRUE)
`, accountID, ownerID); err != nil {
		t.Fatalf("insert USD-M Live credential: %v", err)
	}

	if _, err := runner.Down(context.Background(), 2); err == nil {
		t.Fatal("USD-M Live manual rollback removed persistent account state")
	}
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 20 || latest != 21 {
		t.Fatalf("USD-M Live rollback guard versions = current:%d latest:%d err:%v", current, latest, versionErr)
	}
	if _, err := database.Exec(`DELETE FROM trading_account_credentials WHERE account_id = $1`, accountID); err != nil {
		t.Fatalf("delete USD-M Live credential fixture: %v", err)
	}
	if _, err := database.Exec(`DELETE FROM trading_accounts WHERE id = $1`, accountID); err != nil {
		t.Fatalf("delete USD-M Live account fixture: %v", err)
	}
	if _, err := runner.Down(context.Background(), 1); err != nil {
		t.Fatalf("roll back empty USD-M Live manual migration: %v", err)
	}
	current, latest, versionErr = runner.Versions(context.Background())
	if versionErr != nil || current != 19 || latest != 21 {
		t.Fatalf("USD-M Live rollback versions = current:%d latest:%d err:%v", current, latest, versionErr)
	}
}

func TestM4USDMLiveAutoConstraintsAndDownGuard(t *testing.T) {
	database := openPostgresSchema(t)
	runner, err := New(database)
	if err != nil {
		t.Fatalf("create migration runner: %v", err)
	...
	const accountID = "019de300-0000-7000-8000-000000000001"
	var ownerID, recordID int64
	if err := database.QueryRow(`INSERT INTO users (username) VALUES ('usdm-live-auto-owner') RETURNING id`).Scan(&ownerID); err != nil {
		t.Fatalf("insert USD-M Live auto owner: %v", err)
	}
	if err := database.QueryRow(`
INSERT INTO idempotency_records (user_id, scope, key_hash, request_hash, expires_at, created_at)
VALUES ($1, 'trading-account:create', repeat('3', 64), repeat('4', 64),
        CURRENT_TIMESTAMP + INTERVAL '1 day', CURRENT_TIMESTAMP)
RETURNING id
`, ownerID).Scan(&recordID); err != nil {
		t.Fatalf("insert USD-M Live auto idempotency record: %v", err)
	}
	if _, err := database.Exec(`
INSERT INTO trading_accounts (
    id, owner_user_id, name, market_type, environment, initial_balance,
    paper_fee_rate, leverage, creation_idempotency_record_id,
    status, pause_reason, manual_authorized_at, manual_authorized_by_user_id
) VALUES ($1, $2, 'USD-M Live auto', 'usd_m', 'live', 1000, 0.001, 2, $3,
          'active', '', CURRENT_TIMESTAMP, $2)
`, accountID, ownerID, recordID); err != nil {
		t.Fatalf("insert USD-M Live auto account: %v", err)
	}
	if _, err := database.Exec(`UPDATE trading_accounts SET automation_enabled = TRUE WHERE id = $1`, accountID); err == nil {
		t.Fatal("USD-M Live account enabled automation without administrator and Owner releases")
	}
	if _, err := database.Exec(`
UPDATE trading_accounts
SET automation_enabled = TRUE,
...
WHERE id = $1
`, accountID, ownerID); err == nil {
		t.Fatal("USD-M Live account enabled automation without administrator authorization")
	}
	if _, err := database.Exec(`
UPDATE trading_accounts
SET automation_authorized_at = CURRENT_TIMESTAMP,
...
WHERE id = $1
`, accountID, ownerID); err != nil {
		t.Fatalf("record USD-M Live administrator authorization: %v", err)
	}
	if _, err := database.Exec(`UPDATE trading_accounts SET automation_enabled = TRUE WHERE id = $1`, accountID); err == nil {
		t.Fatal("USD-M Live account enabled automation without Owner auto release")
	}
	if _, err := database.Exec(`
UPDATE trading_accounts
SET automation_enabled = TRUE,
...
WHERE id = $1
`, accountID, ownerID); err != nil {
		t.Fatalf("enable fully released USD-M Live automation: %v", err)
	}

	if _, err := runner.Down(context.Background(), 1); err == nil {
		t.Fatal("USD-M Live auto rollback removed persistent release state")
	}
	current, latest, versionErr := runner.Versions(context.Background())
	if versionErr != nil || current != 21 || latest != 21 {
		t.Fatalf("USD-M Live auto rollback guard versions = current:%d latest:%d err:%v", current, latest, versionErr)
	}
	if _, err := database.Exec(`
UPDATE trading_accounts
SET automation_enabled = FALSE,
...
WHERE id = $1
`, accountID); err != nil {
		t.Fatalf("clear USD-M Live auto release state: %v", err)
	}
	if _, err := runner.Down(context.Background(), 1); err != nil {
		t.Fatalf("roll back empty USD-M Live auto migration: %v", err)
	}
	current, latest, versionErr = runner.Versions(context.Background())
	if versionErr != nil || current != 20 || latest != 21 {
		t.Fatalf("USD-M Live auto rollback versions = current:%d latest:%d err:%v", current, latest, versionErr)
	}
}
