...
func (a *App) assistantToolCatalog(principal *Principal) ([]assistantToolDefinition, map[string]assistantToolExecution) {
	...
	add("platform_overview", "获取平台、数据库和主要业务对象数量的系统概览。", assistantEmptyInputSchema,
		func(ctx context.Context, _ json.RawMessage) (json.RawMessage, *assistantWorkflowProposal, error) {
	...
	add("list_plugins", "列出当前已编译加载的平台插件及贡献类型。", assistantEmptyInputSchema,
		func(context.Context, json.RawMessage) (json.RawMessage, *assistantWorkflowProposal, error) {
	...
	add("list_workflows", "按状态查询最近的平台工作流。", json.RawMessage(`{"type":"object","properties":{"status":{"type":"string","enum":["inactive","active","error"]},"limit":{"type":"integer","minimum":1,"maximum":100,"default":20}},"additionalProperties":false}`),
		func(ctx context.Context, raw json.RawMessage) (json.RawMessage, *assistantWorkflowProposal, error) {
			var input struct {
	...
	add("list_node_capabilities", "查询当前实时工作流节点目录；指定 type 时返回完整 Schema。", json.RawMessage(`{"type":"object","properties":{"type":{"type":"string","maxLength":160}},"additionalProperties":false}`),
		func(_ context.Context, raw json.RawMessage) (json.RawMessage, *assistantWorkflowProposal, error) {
			var input struct {
	...
	add("propose_workflow", "提交一个待用户确认的完整工作流图。该工具只校验方案，不创建、不激活、不运行工作流。", json.RawMessage(`{"type":"object","properties":{"name":{"type":"string","minLength":1,"maxLength":120},"description":{"type":"string","maxLength":500},"graph":{"type":"object"}},"required":["name","description","graph"],"additionalProperties":false}`),
		func(_ context.Context, raw json.RawMessage) (json.RawMessage, *assistantWorkflowProposal, error) {
			var input struct {
			...
			proposal, err := a.workflowProposal(input.Name, input.Description, input.Graph)
			if err != nil {
				return nil, nil, err
			}
			result, _, err := assistantToolJSON(map[string]any{
				"ok": true, "summary": proposal.WorkflowProposalView,
				"instruction": "方案已通过平台校验，请向用户说明并等待其点击确认。",
			}, nil)
	...
	if a.Plugins != nil {
		for _, registered := range a.Plugins.AssistantQueries() {
			...
			add(query.ToolName, query.Descriptor.Description, query.Descriptor.InputSchema,
				func(ctx context.Context, input json.RawMessage) (json.RawMessage, *assistantWorkflowProposal, error) {
...
func (a *App) assistantSystemPrompt(tools []assistantToolDefinition) string {
	...
	return `你是 CoinSphere 平台内置智能助手，只服务超级管理员。你的职责是解释平台、通过工具查询平台数据，并根据用户描述生成工作流方案。
...
工作流规则：必须使用实时节点版本、端口和 JSON Schema。图必须是 schemaVersion=1，包含且仅包含一个主触发器；每个节点必须有 nodeInstanceId、nodeType、nodeVersion、config、position；每条边必须有 edgeId、sourceNodeInstanceId、sourcePort、targetNodeInstanceId、targetPort。密钥字段绝不能写入 graph.config。完成图后必须调用 propose_workflow；平台校验失败时根据错误修正后重试。提案不等于创建，必须提醒用户点击方案卡确认，绝不声称已创建、激活或运行。
...
func assistantToolJSON(value any, err error) (json.RawMessage, *assistantWorkflowProposal, error) {
	if err != nil {
